...
with tab2:
    ...
    st.write("Зріст ($H$) тепер діє як прямий множник масштабу. Чим вища людина, тим більшу площу займає її мандала у фіксованому просторі координат:")
    st.latex(r"S_{global} = \left(\frac{H}{170}\right) \cdot \frac{1}{\frac{S}{5} + 6}")
    st.write("Це забезпечує візуальну різницю розмірів при незмінних межах кадру.")
...
with tab1:
    ...
    with st.sidebar:
        ...
        # Зріст тепер реально впливає на розмір
    ...
    def generate_mandala():
        ...
        # 1. Базовий масштаб
        base_scale = 1 / (S/5 + 6)
        # 2. Множник зросту (170 см = коефіцієнт 1.0)
        height_multiplier = H / 170
        # 3. Підсумковий масштаб
        ...
        # --- ФІКСАЦІЯ КАМЕРИ ---
        # Ми ставимо фіксовану межу (1.3), щоб "зум" не стрибав.
        # Тепер, коли H змінюється, мандала буде візуально рости або зменшуватися в межах цього кадру.
        ax.set_ylim(0, 1.3) 
        ax.set_axis_off()
