...
with tab2:
    ...
    st.write("Зріст ($H$) діє як прямий множник масштабу. Чим вища людина, тим більшу площу займає її мандала у фіксованому просторі:")
    # Формула показує, що H у чисельнику
    st.latex(r"S_{global} = \frac{H}{170} \cdot \frac{1}{\frac{S}{5} + 6}")
    st.write("Межі графічного вікна фіксовані, тому зміна параметра $H$ візуально збільшує або зменшує фігуру.")
...
with tab1:
    ...
    with st.sidebar:
        ...
        # Зріст тепер реально змінює розмір картинки
    ...
    def generate_mandala():
        ...
        # 1. Базовий масштаб (залежить від сну, щоб не вилазило)
        base_scale = 1 / (S/5 + 6)
        # 2. Множник зросту: 170 см = 1.0 (стандарт), 220 см = 1.3 (велика)
