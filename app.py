...
st.write("Епітрохоїда (полярна косинусоїда)")
st.write("""
День народження ($d$) задає частоту коливань, перетворюючи коло на хвилясту зірку. 
Це гармонічне коливання радіуса навколо центру.
""")
st.latex(r"r(\theta) = R_{base} + a \cdot \cos(d \cdot \theta)")
st.write("**Параметри:**")
st.write("""
* **$d$ (Частота):** День народження. Визначає кількість вершин зірки.
* **$a$ (Амплітуда):** Висота променів (у коді вона фіксована для гармонії).
* **$\cos$ (Функція):** Створює плавний перехід між вершинами.
""")
    ...
    st.write("**Математична назва: Спіраль Ферма (Fermat's Spiral)**")
    st.write("""
    Ця спіраль описує розміщення точок (подій) у просторі мандали за принципом золотого перетину.
    """)
    st.latex(r"r_k = c \sqrt{k}, \quad \theta_k = k \cdot 137.5^\circ")
    st.write("**Параметри моделі:**")
    st.write(f"""
    * **$k$ (Індекс):** Порядковий номер точки (від 1 до **Віку $A$**).
    * **$c$ (Константа):** Коефіцієнт щільності (у проєкті $c \approx 0.01$).
    * **Діаметр точки ($D$):** Розраховується на основі **Енергії ($T$)**. 
    ...
    st.write("**Геометрія захисту:**")
    st.write("""
    * **Зріст ($H$):** Визначає параметр частоти $N$. Чим вищий зріст, тим більше зубців у "корони" ($N = H/10$).
    * **Стать ($G$):** Визначає показник степеня **$p$**. Це змінює фізику лінії:
        * Якщо $p < 1$ (чоловіча стать) — лінія стає гострою, створюючи енергію "шипів".
        * Якщо $p > 1$ (жіноча стать) — лінія заокруглюється, створюючи м'які "хвилі".
    """)
