...
with tab2:
    ...
    st.write("Де показник степеня $p = (11 - E) / 2$. При збільшенні $E$ (Впевненості) значення функції між піками падають швидше, що створює візуальний ефект 'гостроти'.")

    # --- СПІРАЛІ ---
    st.subheader("4. Поле спіралей: Ітераційні афінні перетворення")
    st.write("Вік ($A$) визначає кількість ітерацій (шарів). Кожна лінія обертається на кут $\Phi$, що залежить від Енергії ($T$) та Статі ($G$):")
