...
with tab2:
    ...
    st.write("**Базова фігура: Одиничне коло (Unit Circle)**")
    ...
    st.write("**Геометричний примітив: Кільце (Annulus)**")
    st.write("Кільце символізує енергетичний фундамент. Радіус внутрішнього отвору ($r_{hole}$) визначається лінійною функцією від кількості сну ($S$):")
    ...
    st.write("**Базова крива: Полярна косинусоїда (Polar Cosine Wave)**")
    st.write("День народження ($d$) задає частоту коливань, перетворюючи коло на хвилясту зірку:")
    st.latex(r"r(\theta) = R_{base} + A \cdot \cos(d \cdot \theta)")

    # --- 4. ПЕЛЮСТКИ ---
    st.subheader("4. Емоційний контур")
    st.write("**Базова крива: Троянда Гвідо Гранді (Rhodonea Curve)**")
    st.write("Форма пелюсток базується на рівнянні полярної троянди, де місяць ($n$) визначає кількість пелюсток, а Впевненість ($E$) — їхню гостроту (ексцентриситет):")
    st.latex(r"r_{rose} = |\cos(\frac{n}{2}\theta)|^E")
    
    # --- 5. ЕНЕРГІЯ ТА ДОСВІД ---
    st.subheader("5. Поле життєвого досвіду")
    st.write("**Базова крива: Спіраль Ферма (Fermat's Spiral)**")
    st.write("Використано модель філотаксису (як у соняшника). Це параболічна спіраль, що забезпечує найщільніше пакування точок.")
    st.latex(r"r_k = c \cdot \sqrt{k}, \quad \theta_k = k \cdot 137.5^\circ")
    st.write("**Декодування змінних:**")
    st.write("""
    * **$k$ (Лічильник):** Порядковий номер року (від 1 до віку $A$).
    * **$137.5^\circ$:** Золотий кут (Golden Angle), що створює ідеальну спіраль.
    * **$T$ (Енергія):** Визначає візуальну «вагу» (радіус) кожної точки.
    """)

    # --- 6. ЗРІСТ ---
    st.subheader("6. Зовнішня межа")
    st.write("**Базова крива: Модульоване коло (Perturbed Circle)**")
    st.write("Зріст ($H$) визначає частоту захисного бар'єра (кількість зубців). Математично це накладання високочастотного синуса на коло:")
    st.latex(r"r = R_{base} + \alpha \cdot |\sin(N \cdot \theta)|^p")
    st.write("Параметр $p$ (форма зубця) залежить від статі ($G$).")

    # --- 7. ТЕМПЕРАМЕНТ ---
    st.subheader("7. Графічна фізика")
    st.write("Темперамент визначає константи візуалізації:")
    st.write("""
    * **Line Width ($lw$):** Товщина лінії.
    * **Alpha ($\alpha$):** Коефіцієнт прозорості (0.0 - 1.0).
    """)
...
with tab1:
    ...
    # ГЕНЕРАЦІЯ (ОНОВЛЕНА ФУНКЦІЯ З ФАЗОЮ РУХУ)
