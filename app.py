...
st.write("### Математичний проєкт: Алгоритмічна візуалізація персональних даних")
st.markdown("---")

# 2. ВКЛАДКИ
tab1, tab2 = st.tabs(["🚀 Генератор мандали", "📜 Наукове обґрунтування (Математична модель)"])
...
with tab2:
    ...
    # --- ГЛОБАЛЬНИЙ МАСШТАБ ---
    st.subheader("1. Глобальне масштабування та нормалізація")
    ...
    st.write("Це гарантує, що мандала не вийде за межі кадру при будь-яких значеннях зросту ($H$) та сну ($S$).")

    # --- ЦЕНТРАЛЬНЕ КІЛЬЦЕ ---
    st.subheader("2. Центральне кільце (Базовий ресурс)")
    st.write("Кільце символізує внутрішню стабільність. Його ширина та радіус залежать від показника сну ($S$):")
    st.latex(r"r_{in} = \frac{S}{5} \cdot S_{global}, \quad r_{out} = r_{in} + 0.3 \cdot S_{global}")
    st.write("Математично це реалізовано через функцію `fill_between` між двома константами радіуса.")

    # --- ПЕЛЮСТКИ ---
    st.subheader("3. Шар пелюсток: Модифікована 'Троянда Гранді'")
    st.write("Форма пелюсток визначається місяцем народження ($n$) та рівнем рішучості ($E$):")
    st.latex(r"r(\theta) = r_{out} + 0.4 + \left| \cos\left(\frac{n}{2}\theta\right) \right|^p \cdot 2.5 \cdot S_{global}")
    st.write("Де показник степеня $p = (11 - E) / 2$. При збільшенні $E$ (рішучості) значення функції між піками падають швидше, що створює візуальний ефект 'гостроти'.")

    # --- СПІРАЛІ ---
    st.subheader("4. Поле спіралей: Ітераційні афінні перетворення")
    st.write("Вік ($A$) визначає кількість ітерацій (шарів). Кожна лінія обертається на кут $\Phi$, що залежить від Ритму ($T$) та Статі ($G$):")
    st.latex(r"\Phi_{rot} = G \cdot \left( i \cdot \frac{T}{10} \cdot \pi \right)")
    st.write("Це реалізує принцип гвинтової симетрії. Параметр $G$ (стать) діє як інвертор знаку, змінюючи напрямок обертання всієї системи.")

    # --- КОРОНА ---
    st.subheader("5. Зовнішня корона: Гармонійна модуляція")
    st.write("Зовнішній контур відображає взаємодію із середовищем. Частота коливань задається днем народження ($d$):")
    st.latex(r"R_{border} = R_{max} + G \cdot 0.4 \cdot S_{global} \cdot \sin(d \cdot \theta)")
    st.write("Для чоловічої статі ($G=1$) використовується чиста синусоїда (промені), для жіночої ($G=-1$) — модуль синуса (пелюстки).")

    # --- ТЕМПЕРАМЕНТ ---
    st.subheader("6. Графічна інтерпретація темпераменту")
    st.write("Темперамент керує 'фізикою' лінії — її вагою та щільністю:")
    st.write("""
    * **Холерик:** Максимальна насиченість та чіткість лінії.
    * **Флегматик:** Масивна лінія з високою прозорістю (ефект об'єму).
    * **Сангвінік:** Збалансована лінія середньої товщини.
    * **Меланхолік:** Тонка пунктирна лінія (ефект тендітності).
    """)
...
with tab1:
    ...
    def generate_mandala():
        ...
        style_map = {
            ...
            "Флегматик": {"lw": 12.0, "alpha": 0.2, "ls": "-", "f_alpha": 0.15},
        ...
        # 3. ПОЛЕ СПІРАЛЕЙ (Ритм T + Стать G)
