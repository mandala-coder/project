...
with tab2:
    ...
    * **$p$ (Степінь)** — залежить від Впевненості ($E$).
    ...
    # --- 5. СПІРАЛЬ ---
    st.subheader("5. Спіраль (Лінія життя)")
    st.write("Це **Спіраль Ферма** (модель соняшника). Кожна точка символізує один рік життя.")
    st.latex(r"r = c \cdot \sqrt{n}, \quad \theta = n \cdot 137.5^\circ")
    st.write("""
    * **$n$** — номер року (від 1 до твого віку $A$).
    * **$137.5^\circ$** — **Золотий кут**. Це кут, який забезпечує найбільш ефективне упакування насіння в соняшнику.
    """)
    st.write("**Розмір точок (Потужність подій):**")
    st.latex(r"Size \approx Base + (8 \cdot T)")
    st.write("""
    * **$T$ (Енергія)** — визначає діаметр кожної точки.
    * Чим вищий рівень енергії, тим "помітніші" та значущіші роки на малюнку (точки стають більшими).
    """)
