...
with tab2:
    ...
    * Це дозволяє змінювати розмір усієї картинки, змінивши лише одну цифру в коді.
    ...
    # --- 2. ЯДРО ---
    st.subheader("2. Ядро")
    st.write("Центральне коло (або отвір) залежить від того, скільки людина спить.")
    ...
    * **$137.5^\circ$** — **Золотий кут**. Це найважливіша константа природи для опису розміщкення насіння в соняшнику.
    * **$c ≈ 0.12$** — коефіцієнт щільності. Ми підібрали його так, щоб точки стояли тісно (ефект соняшника).
    """)
    ...
    * **$p$ (Степінь форми)**:
        * Для хлопців ($G=1$) константа **$p = 0.5$** (корінь квадратний) - гострі вершини.
        * Для дівчат ($G=-1$) константа **$p = 1.5$** (піднесення до степеня) - плавні хвилі.
    """)
