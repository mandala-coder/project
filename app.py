...
with tab2:
    st.header("📐 Математика моєї особистості")
    st.write("""
    ...
    * **Кут $\\theta$** (напрямок повороту від $0$ до $2\pi$).
    ...
    st.info("Усі формули налаштовані так, щоб малюнок завжди був красивим, незалежно від введених даних.")
    st.markdown("---")

    # --- 1. МАСШТАБУВАННЯ ---
    st.subheader("1. Глобальне масштабування (Розмір)")
    ...
    * Якщо ми пишемо у формулі $2.0$, це означає $2.0 \times SCALE$.
    * Це дозволяє змінювати розмір усієї картинки, змінивши лише одну цифру в коді.
    """)

    # --- 2. ЯДРО ---
    st.subheader("2. Ядро: Сон (Радіус)")
    st.write("Центральне коло (або отвір) залежить від того, скільки я сплю.")
    st.latex(r"r = R_{base} \cdot \left( 1 - \frac{S}{12} \right)")
    st.write("""
    * **$S$** — години сну (змінна).
    * **$12$** — константа (максимальна норма сну).
    * **$R_{base} = 2.0$** — базовий радіус ядра (у одиницях SCALE).
    """)

    # --- 3. ЗІРКА ---
    st.subheader("3. Зірка: День народження (Частота)")
    st.write("Це графік косинуса, який створює хвилі по колу.")
    st.latex(r"r(\theta) = R_{start} + a \cdot \cos(d \cdot \theta)")
    st.write("""
    ...
    * **$R_{start}$** — радіус початку шару.
    """)

    # --- 4. ПЕЛЮСТКИ ---
    st.subheader("4. Квітка: Емоції (Форма)")
    st.write("Використовується рівняння **Троянди Гвідо Гранді**.")
    ...
    * **$p$ (Степінь)** — залежить від Впевненості ($E$).
    ...
    # --- 5. СПІРАЛЬ ---
    st.subheader("5. Спіраль: Досвід (Розподіл)")
    st.write("Це **Спіраль Ферма** (модель соняшника). Вона ідеально заповнює простір точками.")
    st.latex(r"r = c \cdot \sqrt{k}, \quad \theta = k \cdot 137.5^\circ")
    st.write("""
    * **$k$** — номер точки (від 1 до мого віку $A$).
    * **$137.5^\circ$** — **Золотий кут** (ірраціональне число). Це найважливіша константа природи для пакування насіння.
    * **$c \approx 0.12$** — коефіцієнт щільності. Ми підібрали його так, щоб точки стояли тісно (ефект соняшника).
    """)

    # --- 6. МЕЖА ---
    st.subheader("6. Корона: Зріст (Частота) і Стать (Форма)")
    st.write("Зовнішній контур — це складна синусоїда.")
    st.latex(r"r = R + |\sin(N \cdot \theta)|^p")
    st.write("""
    * **$N = H / 10$** — Зріст ділимо на 10. Наприклад, зріст 170 см дає 17 зубців.
    * **$p$ (Степінь форми)**:
        * Для хлопців ($G=1$) константа **$p = 0.5$** (корінь квадратний) $\to$ гострі вершини.
        * Для дівчат ($G=-1$) константа **$p = 1.5$** (піднесення до степеня) $\to$ плавні хвилі.
    """)
