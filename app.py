...
with tab2:
    ...
    * Чим вищий рівень енергії, тим "помітніші" та значущіші роки на малюнку (круги стають більшими).
    ...
    st.write("**Б. Кільця:**")
    st.write("Це концентричні кола розширення ($r = const$).")
    st.write("Кількість таких кіл завжди 7 і вони роблять мандалу більш гармонійною.")
    
   # --- 7. МЕЖА ---
    st.subheader("7. Межа (Епітрохоїда)")
    st.write("Зовнішній контур — це **Епітрохоїда**. Це крива, яку описує точка, закріплена на колі, що котиться по зовнішній стороні іншого нерухомого кола.")
    ...
    st.write("""
    Це метафора взаємодії "Я" (мале коло) і "Світ" (велике коло).
    
    * **$R$ (Світ):** Велике коло, радіус якого залежить від розміру вашої мандали.
    * **$r$ (Я):** Мале коло. Співвідношення $R/r$ визначає кількість пелюсток (залежить від **Зросту**).
    * **$d$ (Характер дотику):** Відстань від центру малого кола до пера.
        * **Для чоловіків ($d \approx r$):** Утворюється **Епіциклоїда** з гострими вершинами (активний, різкий контакт).
        * **Для жінок ($d < r$):** Утворюється плавна хвиля (м'який, адаптивний контакт).
    """)
...
with tab1:
    ...
    def generate_mandala(phase=0):
       ...
       # === 6. МЕЖА (ЕПІТРОХОЇДА) ===
        # Визначаємо, де починається межа (базовий радіус)
        max_dist = max(points_r) if points_r else r_grid_end
        base_radius = max(r_grid_end, max_dist + 0.5 * SCALE)

        # ПАРАМЕТРИ ЕПІТРОХОЇДИ
        # N - кількість пелюсток (залежить від зросту H). 
        # Наприклад, зріст 170 -> 17 пелюсток.
        N_lobes = int(H / 10)
        
        # Математика:
        # Епітрохоїда утворюється коченням кола радіусом r_roll по колу R_fix.
        # Відношення R_fix / r_roll = N_lobes визначає кількість пелюсток.
        # R_sum = R_fix + r_roll — це відстань від центру до центру малого кола.
        
        r_roll = base_radius / (N_lobes + 1)  # Радіус малого (рухомого) кола
        R_fix = r_roll * N_lobes              # Радіус великого (нерухомого) кола
        
        # d_param - відстань від центру малого кола до точки малювання (перо).
        # Це визначає форму "зубців":
        if G == 1: 
            # Чоловічий варіант (Епіциклоїда): точка на ободі кола.
            # Дає гострі вершини (cusps).
            d_param = r_roll * 1.0 
        else:      
            # Жіночий варіант (Подовжена епітрохоїда): точка всередині кола.
            # Дає плавні, хвилясті лінії.
            d_param = r_roll * 0.6 

        # Генерація точок (параметричне рівняння)
        # Беремо більше точок (2000) для плавності
        t_epi = np.linspace(0, 2 * np.pi, 2000) 
        
        # Формули епітрохоїди (у декартових координатах)
        # Додаємо phase до t_epi для обертання всієї фігури
        ...
        # Перетворення назад у полярні координати для Matplotlib
        r_epi = np.sqrt(x**2 + y**2)
        theta_epi = np.arctan2(y, x)
        
        # Сортування кутів, щоб графік малювався послідовно (уникнення артефактів)
        sort_idx = np.argsort(theta_epi)
        
        # Малюємо
        # Додаємо breath до радіуса для пульсації
