...
with tab2:
    ...
    st.write("Для відображення віку використано модель **філотаксису** (золотий перетин). Це дозволяє розмістити велику кількість точок без хаосу.")
    st.latex(r"r_n = c \sqrt{n}, \quad \theta_n = n \times 137.5^\circ")
    st.write("**Параметризація:**")
    st.write("""
    * **Вік ($A$):** Визначає точну кількість точок-подій.
    * **Енергія ($T$):** Визначає два фактори: 
        1. Щільність енергетичної сітки (кількість радіальних променів).
        2. Розмір самих точок (маса події).
    """)
