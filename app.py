...
with tab2:
    ...
    st.write("Для забезпечення композиційної цілісності простір нормовано. Всі об'єкти вписані в одиничне коло:")
    st.latex(r"R_{max} \approx 1.5 \cdot \text{Unit}")
    st.write("Це дозволяє порівнювати структуру особистостей без візуального спотворення масштабу.")

    # --- 2. СОН ---
    st.subheader("2. Центральне ядро")
    st.write("Кільце символізує енергетичний фундамент. Радіус внутрішнього отвору ($r_{hole}$) обернено пропорційний кількості сну ($S$):")
    st.latex(r"r_{hole} = R_{core} \cdot \left(1 - \frac{S}{12}\right)")
    st.write("При $S=12$ (максимум) отвір зникає, утворюючи суцільний диск. У разі дефіциту сну ядро стає порожнім кільцем.")

    # --- 3. ДЕНЬ НАРОДЖЕННЯ ---
    st.subheader("3. Гармонічний резонанс")
    st.write("Другий шар — це гармонічна зірка. День дати народження ($d$) визначає **частоту** коливань (кількість вершин):")
    st.latex(r"r(\theta) = R_{base} + A \cdot \cos(d \cdot \theta)")

    # --- 4. ПЕЛЮСТКИ ---
    st.subheader("4. Емоційний контур")
    st.write("Форма пелюсток визначається місяцем ($n$), а їхня геометрія модулюється Впевненістю ($E$):")
    st.latex(r"r_{rose} = |\cos(\frac{n}{2}\theta)|^E")
    st.write("Висока впевненість перетворює плавні лінії на гострі вектори.")

    # --- 5. ЕНЕРГІЯ ТА ДОСВІД ---
    st.subheader("5. Поле життєвого досвіду")
    st.write("Використано модель філотаксису (розміщення насіння в соняшнику). Це ідеальна упаковка подій у просторі пам'яті.")
    
    # ЗМІНА: Вводимо індекс k, який біжить від 1 до віку A
    st.latex(r"r_k = c \cdot \sqrt{k}, \quad \theta_k = k \cdot 137.5^\circ")
    st.latex(r"k \in \{1, 2, ..., A\}")
    
    st.write("**Декодування змінних:**")
    st.write("""
    * **$k$ (Лічильник):** Порядковий номер року. Пробігає значення від 1 до вашого **Віку ($A$)**.
    * **$c$ (Константа):** Масштабний коефіцієнт щільності спіралі.
    * **$T$ (Енергія):** Визначає фізичний **розмір (вагу)** кожної точки на графіку ($Size \sim T$).
    """)
    
    # --- 6. ЗРІСТ ---
    st.subheader("6. Зовнішня межа")
    st.write("Зріст ($H$) визначає частотну характеристику захисного бар'єра (кількість зубців межі):")
    st.latex(r"N_{peaks} = \frac{H}{10}")
    st.write("Геометрія зубців (гострі чи м'які) залежить від Статі ($G$).")
