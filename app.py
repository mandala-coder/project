...
with tab2:
    ...
    st.write("Центр мандали — кільце. Його внутрішній радіус $r_{hole}$ залежить від дефіциту сну:")
    st.latex(r"r_{hole} = R_{core} \cdot \left(1 - \frac{S}{12}\right)")
    st.write("Якщо сну достатньо ($S \ge 12$), $r_{hole} \to 0$ (суцільний диск).")
...
with tab1:
    ...
    def generate_mandala():
        ...
        R_core_max = 0.3
        # Логіка дірки: S=12 -> hole=0. S=0 -> hole=R_core_max
        hole_r = R_core_max * (1 - (S / 12.0))
        hole_r = max(0.0, hole_r) # Без від'ємних значень
