...
with tab2:
    ...
    st.write("Щоб уникнути виходу за межі кадру, простір координат зафіксовано для максимально можливого зросту ($H_{max}=220$). Зріст користувача визначає заповнення цього простору:")
    st.latex(r"S_{global} = \frac{H}{160} \cdot \frac{1}{\frac{S}{5} + 6}")
    st.write("Висока людина створює експансивну мандалу, що прагне до меж кадру, тоді як невисокий зріст формує компактну, сконцентровану структуру.")
    ...
    * **Жіночий вектор ($p=1.5$):** створює органічність.
    ...
    st.write("""
   * **Холерик:** Товста і насичена лінія - символ енергії та сили.
    * **Флегматик:** Широка, але напівпрозора лінія - символ спокою та врівноваженості.
    * **Сангвінік:** Чітка лінія середньої товщини - символ балансу та активності.
    * **Меланхолік:** Тоненький пунктир - символ чутливості та ніжності.
    """)
...
with tab1:
    ...
    def generate_mandala():
        ...
        # --- ЛОГІКА МАСШТАБУВАННЯ (ЗРІСТ H) ---
        base_scale = 1 / (S/5 + 6)
        
        # H / 160 дає кращий баланс. 
        # При 160 см = коефіцієнт 1.0. При 220 см = 1.375.
        ...
        # --- ФІКСАЦІЯ КАМЕРИ НА МАКСИМУМ ---
        # Ми ставимо ліміт 2.2.
        # Навіть при зрісті 220 см та максимальному сні радіус мандали не перевищить ~2.0.
        # Тому при 220 см вона буде виглядати великою, але не обріжеться.
        ax.set_ylim(0, 2.2)
        ax.set_axis_off()
