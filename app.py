...
with tab2:
    ...
    * **Менше сну:** З'являється отвір, який росте при дефіциті сну.
    ...
    # --- ЕПІТРОХОЇДА (ЗРІСТ - ПОВЕРНУЛИ) ---
    st.subheader("2. Гармонічний контур (Зріст)")
    st.write("Зріст ($H$) визначає **частоту** гармонічної фігури (Епітрохоїди), що оточує ядро. Це каркас особистості.")
    st.latex(r"r_{epi}(\theta) = R_{base} + A \cdot \cos\left( \frac{H}{10} \cdot \theta \right)")
    st.write("Вищі люди створюють більш складну багатогранну зірку (більше вершин), компактний зріст формує стійкі фігури (трикутник/квадрат).")
...
with tab1:
    ...
    with st.sidebar:
        ...
        # Зріст впливає на Епітрохоїду
        H = st.slider("Зріст (см) -> Кількість кутів", 100, 220, 170)
