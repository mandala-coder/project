...
with tab1:
    ...
    def generate_mandala(phase=0):
     ...
     # === 6. МЕЖА (ЕПІТРОХОЇДА) ===
        # Визначаємо базу для малювання межі
        max_dist = max(points_r) if points_r else r_grid_end
        base_radius = max(r_grid_end, max_dist + 0.5 * SCALE)

        # Параметри епітрохоїди
        N_lobes = int(H / 10) # Кількість пелюсток від зросту
        
        # Радіуси кіл для генерації кривої
        r_roll = base_radius / (N_lobes + 1)
        R_fix = r_roll * N_lobes
        
        # Характер дотику (гострий або плавний)
        if G == 1: 
            d_param = r_roll * 1.0 # Гострі вершини (чоловічий)
        else:      
            d_param = r_roll * 0.6 # Плавні хвилі (жіночий)

        # Генерація точок через параметричні рівняння
        ...
        # Малюємо межу
        # Сортувати theta не потрібно, бо ми малюємо параметричну криву, але
        # для ax.plot у полярних координатах порядок точок має значення.
        # Найпростіший спосіб уникнути ліній через центр - малювати сегментами або просто sorted,
        # але для замкненої епітрохоїди сортування за кутом працює коректно, якщо вона зірчаста.
