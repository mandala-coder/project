...
with tab2:
    ...
    # --- ЦЕНТРАЛЬНЕ КІЛЬЦЕ ---
    st.subheader("2. Центральне кільце (Внутрішній ресурс)")
    ...
    # --- КОРОНА ---
    st.subheader("5. Зовнішня межа: Нелінійна модуляція")
    st.write("Щоб уникнути надмірної декоративності ('воланчиків') та підкреслити характер вектора, використано степеневу модуляцію:")
    st.latex(r"R_{border} = R_{max} + 0.5 \cdot S_{global} \cdot |\sin(d \cdot \theta)|^p")
    st.write("""
    * **Чоловічий вектор ($p=0.4$):** створює жорсткі, кристалічні шипи (структурність).
    * **Жіночий вектор ($p=1.5$):** створює м'які, округлі пелюстки (органічність).
    """)
